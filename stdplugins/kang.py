...
async def _(event):
    ...
    packname = f"@Archer Zen pack"
    packshortname = f"archer_Zenpack{userid}"  # format: Uni_Borg_userid

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "trans_Sticker.png"
    file = await borg.download_file(reply_message.media)
    uploaded_sticker = None
    if is_a_s:
        file_ext_ns_ion = "AnimatedSticker.tgs"
        uploaded_sticker = await borg.upload_file(file, file_name=file_ext_ns_ion)
        packname = f"Zen_AnimatedStickers1"
        packshortname = f"Zen_Animated1"  # format: Uni_Borg_userid
    ...
    await event.edit(f"Processing.....[here](t.me/addstickers/{packshortname})")
